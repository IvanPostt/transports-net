...
class UserLoginForm(AuthenticationForm):
    ...
    password = forms.CharField(widget=forms.PasswordInput(attrs={'class': 'form-control py-4', 'placeholder': 'Пароль'}))
    class Meta:
        model = User
...
class UserRegisterForm(UserCreationForm):
    username = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control py-4', 'placeholder': 'Имя'}))
    city = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control py-4', 'placeholder': 'Введите Город Проживания'}))
    birth_date = forms.DateField(widget=forms.DateInput(attrs={'class': 'form-control py-4', 'placeholder': 'Дата рождения'}))
    ...
    password2 = forms.CharField(widget=forms.PasswordInput(attrs={'class': 'form-control py-4', 'placeholder': 'Подтверждения пароля'}))
    class Meta:
        model = User
    ...
    #     expiration = now() + timedelta(hours=72)
    #     record = EmailVerification.objects.create(code=uuid.uuid4(), user=user, expiration=expiration)
    #     record.send_verification_email(user)
    #     return user
